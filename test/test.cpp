...
namespace all_tests
{
	...
	public:
		TEST_METHOD(test_01)
		{
			...
			copy(it, end, std::back_inserter(v));
		...
		TEST_METHOD(test_03a)
		{
			...
			transform(v.begin(), v.end(), v.begin(), [](int& i) {return i * i * i; });
		...
		TEST_METHOD(test_06)
		{
			...
			std::vector<int>::iterator it = find_if(std::begin(v), std::end(v), [](int n) {
		...
		TEST_METHOD(test_07b)
		{
			...
			std::replace_if(std::begin(s), std::end(s), [](char c) {
		...
		TEST_METHOD(test_08a)
		{
			...
			std::vector<double>::iterator it = std::remove(std::begin(v), std::end(v), 1e10);
		...
		TEST_METHOD(test_08b)
		{
			...
			//delete all vowels 
			std::string::iterator it = std::remove_if(std::begin(s), std::end(s), [](char c) {
				if (c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u') return true;
				return false;
				});
		...
		TEST_METHOD(test_09)
		{
			...
			std::sort(std::begin(v), std::end(v), [](exam const& ex1, exam const& ex2) {
				if (ex1.grade != ex2.grade) return ex1.grade > ex2.grade;
				else return ex1.points > ex2.points;
				});
		...
		TEST_METHOD(test_12)
		{
			...
			std::vector<int> atp_diff;
			std::sort(std::begin(atp_points), std::end(atp_points));
			std::adjacent_difference(atp_points.begin(), atp_points.end(), std::back_inserter(atp_diff));
			auto smallest_difference = *std::min_element(std::begin(atp_diff) + 1, std::end(atp_diff)); //
